...
FOUNDATION_EXPORT double MikiWeakTimeVersionNumber;
FOUNDATION_EXPORT const unsigned char MikiWeakTimerVersionString[];
NS_ASSUME_NONNULL_BEGIN
/**
 `MikiWeakTimer` behaves similar to an `NSTimer` but doesn't retain the target.
 This timer is implemented using GCD, so you can schedule and unschedule it on arbitrary queues (unlike regular NSTimers!)
 */
@interface MikiWeakTimer : NSObject
 ...
 * @param repeats if `YES`, `selector` will be invoked on `target` until the `MikiWeakTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
/**
 * Creates an `MikiWeakTimer` object and schedules it to start ticking inmediately.
 */
 ...
 * @note `MikiWeakTimer` won't invoke the `selector` on `target` again after calling this method.
...
@interface MikiWeakTimer(Background)

/**
 * 创建脱离后台管理策略的MikiWeakTimer,
 * @note 参数与- (id)initWithTimeInterval:(NSTimeInterval)timeInterval一致
 */
...
/**
 * 创建脱离后台管理策略的MikiWeakTimer，立即执行。
 * @note 参数与+ (instancetype)scheduledTimerWithTimeInterval:(NSTimeInterval)timeInterval
 * 一致
 */
